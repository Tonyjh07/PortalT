...
| ESXi 反代补 /folder /nfc 端点（数据存储上传修复） | ✅ 完成 | 2026-08-20 | 修复经 Caddy 反代后 Host Client 数据存储上传失败（进度条跑完、最终确认失败；内网直连/FRP 透传正常）：上传走 hostd `PUT /folder/<path>?dcPath=ha-datacenter&dsName=..`（数据存储浏览/下载同端点），OVF/OVA 导入走 HttpNfcLease `/nfc/<uuid>/<file>`，两者缺代理均落到门户 SPA 兜底；默认规则新增 `handle /folder*`、`handle /nfc*` 反代块（带鉴权闸口），原默认改名 `DefaultESXIAdminCaddyRulesV3`（缺 folder/nfc）作历史值，seed 迁移对归一化等于 V1/V2/V3 的旧默认统一升级；`caddy_test.go` 单行块语法适配新版 Caddy（拒绝单行 `{}`/`{ ... }`，改多行块）；`cmd/server` 新增 V3 迁移用例 + 文档同步（plugins.md/external-access.md）+ `go test ./... -count=1` 全绿 |
