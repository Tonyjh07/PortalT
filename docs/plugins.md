...
- 环境变量：
  - `PLUGIN_CADDY_DIR`：插件 Caddy 规则目录（默认部署机 `/etc/caddy/plugins.d`）；
    **为空 = 本地 dev 无 Caddy**：保存返回告警"规则已保存但未落盘"，`POST /caddy-reload` 返回 503，
    均明确提示而非静默空操作
  - `CADDY_RELOAD_CMD`：reload 命令（默认 `systemctl reload caddy`）；为空时规则可落盘但不会
    热生效，保存/重载均返回告警（重载响应 `reloaded:false`）；命令执行失败时规则已落盘但提示
    "将随下次 reload 生效"
...
- 管理页头部"**重载 Caddy**"按钮走 `POST /api/v1/plugins/caddy-reload`（`plugin:manage`）：
  ...
  `PLUGIN_CADDY_DIR` 未配置时返回 503（规则不会落盘），`CADDY_RELOAD_CMD` 未配置时
  返回 200 + 告警且 `reloaded:false`。
...
- 现状说明：`esxi-admin` 插件的 `caddy_rules` 默认值（`DefaultESXIAdminCaddyRules`）是 ESXi
  ...
  `/folder*`、`/nfc*` 等 handle，目标主机由 `{env.ESXI_UPSTREAM}` 运行时解析）；**内置
  `caddy/Caddyfile` 已瘦身、不再包含 ESXi handle**——停用/删除该插件即移除规则文件，
  `/esxi/*` 不再反代（访问自然收回）。
  `/ha-nfc/*` 是 ESXi NFC 端点，Host Client 导出 OVF/OVA 时对其 HEAD 测文件 size，漏配会落到
  前端 SPA 兜底导致导出报 `Required property not defined: size`。
  `/folder*` 是 hostd 数据存储访问端点（`PUT /folder/<path>?dcPath=ha-datacenter&dsName=..`
  上传、GET 下载/目录列表）：漏配时上传请求落到 SPA 兜底，表现即"进度条跑完、最终确认失败"。
  `/nfc*` 是 HttpNfcLease 端点（`/nfc/<uuid>/<file>`），OVF/OVA 导入（vCenter 直连）与
  磁盘级文件拉取走该路径，漏配时导入失败。
...
历史默认（无鉴权版 `DefaultESXIAdminCaddyRulesV1`、缺 `/ha-nfc` 版
  `DefaultESXIAdminCaddyRulesV2`、缺 `/folder` `/nfc` 版 `DefaultESXIAdminCaddyRulesV3`）
  会被启动 seed 自动升级为新默认值（精确匹配才覆盖，保留管理员自定义）。
